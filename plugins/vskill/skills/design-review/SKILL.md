...
If the target project has `CLAUDE.md` at its root, that file **outranks this skill**. When a project convention conflicts with a check below (e.g. a different spacing scale, a bespoke icon set), honour the project's convention and flag the contradiction in the report rather than marking it as a defect.
...
| Check | Good | Bad |
|-------|------|-----|
| **Semantic colour** | Using Nuxt UI / design tokens (`var(--ui-text)`, `var(--ui-text-muted)`, `var(--ui-border)`, theme classes like `text-primary`) | Raw hex / rgb (`color: #333`, `background: rgb(0 0 0 / .5)`) hardcoded in scoped SCSS |
...
| **Icon style** | One icon family (`i-material-symbols:*` on this stack), consistent size and stroke | Mixed icon families, different sizes, some filled some outlined |
...
Write findings to `docs/design-review.md` at the project root:

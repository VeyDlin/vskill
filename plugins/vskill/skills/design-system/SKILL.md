...
Analyse an existing website, HTML file, or screenshot and synthesise a semantic design system into a `DESIGN.md` file. The output is optimised for feeding into page/component generation (including the `design-prototype` autonomous loop).

## Priority rule

If the target project has `CLAUDE.md` at its root, that file **outranks this skill**. When the extracted tokens conflict with the project's existing design system (Nuxt UI theme, CSS variables, spacing scale), preserve the project system and report the source's tokens as a reference/comparison — do not overwrite the project's canonical design.
...
- Preparing `docs/DESIGN.md` before running `design-prototype` or generating pages
...
1. **CSS custom properties** — `:root { --ui-primary: #hex; --ui-text: #hex; }` or `@theme` blocks
2. **Framework config** — Nuxt UI theme object, Tailwind config `<script>`, or `@theme` in `<style>`
3. **Inline styles** — `style="color: #hex"` or `style="background: #hex"`
4. **Utility classes** — `bg-primary`, `text-muted`, or Tailwind-style `bg-blue-600` (map to semantic role)
...
| Heading weights | Utility classes (`font-bold`, `font-semibold`) or explicit `font-weight` |
| Body size | Base `font-size` on `<body>` or root |
| Line height | `line-height` CSS or utility classes (`leading-*`) |
| Letter spacing | `letter-spacing` CSS or utility classes (`tracking-*`) |
...
- **Max content width** — look for explicit `max-width` or utility classes (`max-w-*`)
...
|-------------|---------------|
| `border-radius: 12px` / `rounded-xl` | "Softly rounded corners (12px)" |
| `box-shadow: 0 4px 6px …` / `shadow-md` | "Subtle elevation with diffused shadow" |
| `#1E40AF` | "Deep Ocean Blue (#1E40AF) for primary actions" |
| `padding: 4rem 0` / `py-16` | "Generous section spacing with breathing room" |

### Step 5: Write DESIGN.md

Output the file to `docs/DESIGN.md` at the project root (or user-specified path).

Structure the document with these six sections:

1. **Visual Theme & Atmosphere** — mood, vibe, philosophy (2-3 sentences)
2. **Colour Palette & Roles** — table with role, name, hex, and usage notes (include light/dark variants if present)
3. **Typography** — font families, weights, sizes, line heights, letter spacing
4. **Component Styles** — buttons, cards, nav, forms (describe shape, elevation, states)
5. **Layout Principles** — max content width, section spacing, grid, whitespace philosophy
6. **Design System Notes for Generation** — a condensed copy-paste block (plain prose + a short token list) that can be pasted into a prompt when asking Claude to build a page in this system
...
- **Sites with a published config (Tailwind, Nuxt UI, CSS custom properties) are easiest** — tokens are declared intentionally
- **Google Fonts links are gold** — they specify exact families and weights
- **CSS custom properties are reliable** — they represent intentional design tokens; look for `--ui-*`, `--primary`, `--color-*`
- **Utility classes need interpretation** — `bg-slate-900` or `bg-primary` needs mapping to a semantic role

...
If the target project has `CLAUDE.md` at its root, that file **outranks this skill**. Honour the project's stated routing model, design system, and UX conventions when judging findings — report deviations from the project's own rules, not from the generic defaults below.
...
1. Check CLAUDE.md, README, `package.json` `homepage` field, or a deploy config (`wrangler.jsonc`, `netlify.toml`, `vercel.json`)
2. Fall back to a running dev server (Vite default `http://localhost:5173`, or `lsof -i :5173 -i :3000 -i :8787 -t` on Unix)
3. Ask the user
...
1. **Router config** — read the app's route definitions (Vue Router config under `src/router/` on this stack; React Router, TanStack Router, Next.js app dir on other stacks)
...
Monitor network responses across the entire session. Visual browsing misses API failures — vue-query, TanStack Query, and SWR swallow errors and show empty/loading states instead. A "No results found" might actually be a 403.

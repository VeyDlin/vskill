...
If the target project has `CLAUDE.md` at its root, that file **outranks this skill**. If the project defines its own breakpoints or declares a mobile-first / desktop-only stance, honour it — align the checked widths with the project's stated breakpoints and report findings against the project's rules, not the generic matrix below.

...
If the target project has `CLAUDE.md` at its root, that file **outranks this skill**. An extract is a reference library produced *from* an external app — it should never overwrite or edit the project's own design system or pattern files; write it to the dedicated extracts folder only.
